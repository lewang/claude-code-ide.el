...
;; For other environment variables not related to API providers:
...
Signals an error if terminal fails to initialize."
  ...
  (let* ((claude-cmd (claude-code-ide--build-claude-command continue resume session-id))
    ...
    (when provider-env-vars
      (claude-code-ide-debug "Provider environment variables: %s"
                             (claude-code-ide--obfuscate-env-vars provider-env-vars)))
